...
print john.emails
carl_address = session.query(Email).filter_by(address='carl@nospam.net').first()
...
print session.query(Email).filter_by(address='mary@example.com').all()
print session.query(User).filter_by(age=42).all()
...
print session.query(Email).filter_by(user_id=2).all()